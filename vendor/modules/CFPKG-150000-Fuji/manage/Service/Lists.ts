...
const API = 'longways/account-system/lists';
...
export namespace typeLists {
  ...
  export type response_f = { list: item_f[]; profile: profile; paginator: object };

  export type response = { list: item[]; profile: profile; paginator: object };
  ...
  if (res.list !== null && res.list.length) {
    const list = res.list.map(item => ({
      ...
      profile: res.profile.p3_account_profile.find(
        e => e.cp9_cape_p3_ant_id === item.p3_ant_id,
